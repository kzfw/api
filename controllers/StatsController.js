...
router.get('/ins', getUser, auth(['atm', 'datm', 'ta', 'ins', 'mtr', 'ia']), async (req, res) => {
	try {
		...
		const controllersWithoutTraining = allHomeControllers.filter((c) => cidsWithoutTraining.includes(c.cid)).filter(c => !c.certCodes.includes('zfw'));
	...
	try {
		...
		for(let user of users) {
			...
			userData[user.cid] = {
				...
				protected: user.isStaff || [1202744, 1190916, 1313538, 1118012].includes(user.cid) || user.absence.some(a => !a.deleted && new Date(a.expirationDate) > new Date() && a.controller === user.cid)
			}
		}
